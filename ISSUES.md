...
## Issue 21: TypeError in api/photos-controller with `null` photo records

**Opened By:** Jamie – Jun 17 2025  
**Status:** Resolved

### Description  
Some `photos.json` files generated by _osxphotos_ contained `null` rows
(appears when Photos.app marks assets as “buried”).  
The rename loop attempted `p.persons = …` on these falsy items, throwing:

```
TypeError: Cannot set properties of undefined (setting 'persons')
```

### Fix (Commit `fix(api/photos): guard against null …`)

1. Filter out non‑object entries immediately after reading `photos.json`.
2. Extra defensive `p.persons ?? []` when building the persons map.
3. Added regression test (`photos-controller.test.js`) to ensure the
   controller survives a `null` hole.

### Verification  
`npm run test` now passes and the album API returns valid JSON without
errors even when the source file contains placeholders.
