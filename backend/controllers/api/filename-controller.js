...
export async function getPeopleByFilename(req, res) {
  try {
    ...
    if (!albumUUID) {
      const lookupResult = await findAlbumUUIDByFilename(albumsDir, {
        filename,
        lookupKey,
      });
...
async function getAlbumPhotosJsonPath(albumUUID) {
  ...
  const legacyPath = path.join(getLocalRoot(), "albums", albumUUID, "photos.json");
...
async function findAlbumUUIDByFilename(albumsDir, lookup) {
  const { filename, lookupKey } = lookup;
...
function normalizeLookupFilename(rawInput) {
  const str =
    typeof rawInput === "string" ? rawInput : rawInput?.toString() ?? "";
  ...
  const candidate = basename.normalize("NFC");
